...
public class v1_9_R2 implements NMS {
    ...
    public void setBlockFast(org.bukkit.World world, int x, int y, int z, int blockId, byte data, boolean applyPhysics) {
        ...
        net.minecraft.server.v1_9_R2.Chunk nmsChunk = nmsWorld.getChunkAt(x >> 4, z >> 4);
        IBlockData ibd = net.minecraft.server.v1_9_R2.Block.getByCombinedId(blockId + (data << 12));
    ...
    public void sendChunk(Chunk chunk, List<Player> players) {
        Bukkit.getScheduler().runTaskAsynchronously(IridiumChunkBusters.getInstance(), () -> chunk.getWorld().getPlayers().forEach(player -> {
            net.minecraft.server.v1_9_R2.PacketPlayOutMapChunk packetPlayOutMapChunk = new net.minecraft.server.v1_9_R2.PacketPlayOutMapChunk(((org.bukkit.craftbukkit.v1_9_R2.CraftChunk) chunk).getHandle(), 65535);
            ((org.bukkit.craftbukkit.v1_9_R2.entity.CraftPlayer) player).getHandle().playerConnection.sendPacket(packetPlayOutMapChunk);
        }));
    }

    @Override
    public void sendActionBar(Player player, String message) {
        net.minecraft.server.v1_9_R2.IChatBaseComponent iChatBaseComponent = net.minecraft.server.v1_9_R2.IChatBaseComponent.ChatSerializer.a(ChatColor.translateAlternateColorCodes('&', "{\"text\":\"" + message + "\"}"));
        PacketPlayOutChat packetPlayOutChat = new PacketPlayOutChat(iChatBaseComponent, (byte) 2);
        ((org.bukkit.craftbukkit.v1_9_R2.entity.CraftPlayer) player).getHandle().playerConnection.sendPacket(packetPlayOutChat);
    }
}
