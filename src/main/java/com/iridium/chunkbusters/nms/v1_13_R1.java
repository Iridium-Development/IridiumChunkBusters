...
public class v1_13_R1 implements NMS {
    ...
    public void sendChunk(Chunk chunk, List<Location> blocks, List<Player> players) {
        net.minecraft.server.v1_13_R1.Chunk nmsChunk = ((CraftChunk) chunk).getHandle();
    ...
    public void sendChunk(Chunk chunk, List<Player> players) {
        Bukkit.getScheduler().runTaskAsynchronously(IridiumChunkBusters.getInstance(), () -> chunk.getWorld().getPlayers().forEach(player -> {
            net.minecraft.server.v1_13_R1.PacketPlayOutMapChunk packetPlayOutMapChunk = new net.minecraft.server.v1_13_R1.PacketPlayOutMapChunk(((org.bukkit.craftbukkit.v1_13_R1.CraftChunk) chunk).getHandle(), 65535);
            ((org.bukkit.craftbukkit.v1_13_R1.entity.CraftPlayer) player).getHandle().playerConnection.sendPacket(packetPlayOutMapChunk);
        }));
    }
}
