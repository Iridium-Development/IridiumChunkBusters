...
public class v1_11_R1 implements NMS {
    ...
    public void setBlockFast(World world, int x, int y, int z, int blockId, byte data, boolean applyPhysics) {
        ...
        net.minecraft.server.v1_11_R1.Chunk nmsChunk = nmsWorld.getChunkAt(x >> 4, z >> 4);
        IBlockData ibd = net.minecraft.server.v1_11_R1.Block.getByCombinedId(blockId + (data << 12));
    ...
    public void sendChunk(Chunk chunk, List<Player> players) {
        Bukkit.getScheduler().runTaskAsynchronously(IridiumChunkBusters.getInstance(), () -> chunk.getWorld().getPlayers().forEach(player -> {
            net.minecraft.server.v1_11_R1.PacketPlayOutMapChunk packetPlayOutMapChunk = new net.minecraft.server.v1_11_R1.PacketPlayOutMapChunk(((org.bukkit.craftbukkit.v1_11_R1.CraftChunk) chunk).getHandle(), 65535);
            ((org.bukkit.craftbukkit.v1_11_R1.entity.CraftPlayer) player).getHandle().playerConnection.sendPacket(packetPlayOutMapChunk);
        }));
    }
    ...
    public void sendActionBar(Player player, String message) {
        ...
        ((org.bukkit.craftbukkit.v1_11_R1.entity.CraftPlayer) player).getHandle().playerConnection.sendPacket(packetPlayOutTitle);
    }
}
